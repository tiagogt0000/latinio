...
export function harness(){
 const sheets=new Map();let locked=false;
 function makeSheet(name){const rows=[];const sheet={rows,getLastRow:()=>rows.length,getRange(r,c,n=1,m=1){const range={getValue:()=>rows[r-1]?.[c-1],getValues:()=>Array.from({length:n},(_,i)=>Array.from({length:m},(_,j)=>rows[r-1+i]?.[c-1+j]??'')),setValues(values){values.forEach((row,i)=>{rows[r-1+i]??=[];row.forEach((v,j)=>rows[r-1+i][c-1+j]=v);});return range;},setFontWeight(){return range;},setBackground(){return range;},setFontColor(){return range;}};return range;},setFrozenRows(){},setColumnWidths(){},setColumnWidth(){}};sheets.set(name,sheet);return sheet;}
 ...
 ctx.ensureLog_('admin');return {request:request=>ctx.latinioApi({token,...request}),ctx,rows:sheets.get('Änderungen').rows,sheets,props};
}
