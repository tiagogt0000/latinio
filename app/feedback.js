...
export function answerFeedback(feedback){
  const rows=feedback.rows.filter(row=>row.kind!=='empty').map(row=>{
    ...
    return `<div class="graded-answer ${wrong?'incorrect':'correct'}"><div class="graded-content"><span class="answer-mark" aria-label="${wrong?'Falsch':'Richtig'}">${wrong?'✕':'✓'}</span><span>${wrong?`<s>${escape(row.answer)}</s>`:escape(row.answer)}</span></div>${wrong?`<button type="button" class="typo-link" data-action="correct-evaluation" data-index="${row.index}">Auswertungsfehler korrigieren</button>`:''}</div>`;
  });
  return rows.join('');
}
