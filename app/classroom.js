...
export function classroomView(data,query,h){return `<div class="page-heading"><div><div class="eyebrow">SCHNELL NACHSCHLAGEN</div><h1>Unterricht</h1><p class="muted">Vokabeln suchen – ohne Test oder Bewertung.</p></div></div><section class="card"><label for="classroom-search">Lateinisches Wort oder deutsche Bedeutung</label><input id="classroom-search" type="search" value="${h(query)}" placeholder="Zum Beispiel vox, vocis oder Stimme" autocomplete="off" autocapitalize="none" spellcheck="false" style="width:100%;margin-top:12px"><p class="small muted">Durchsucht deine gespeicherten Sammlungen, auch offline. Auch inaktive Sammlungen und zusätzliche Suchformen werden durchsucht. Automatisch abgeleitete Formen sind mögliche Treffer, keine vollständige Formenbestimmung.</p></section><section id="classroom-results" class="lookup-results" aria-live="polite">${classroomResults(data,query,h)}</section>`;}
