...
export function accountCard(profile){return `<section class="card"><h2>Dein Profil</h2><p>${h(profile.role==='admin'?'Tiago':profile.name)}</p><p class="small muted">${profile.role==='admin'?'Administrator':h(profile.email)}</p>${profile.role==='admin'?'<button class="button secondary wide" data-action="account-profiles">Profile verwalten</button>':''}${profile.role!=='admin'?'<p class="small muted">Deine Lehrkraft sieht ein kurzes Aktivitätsprotokoll mit Datum, Uhrzeit und allgemeinen Aktionen. Es enthält keine einzelnen Wörter, Antworten oder Richtig/Falsch-Ergebnisse. Es zeigt die letzten 90 Tage (höchstens 200 Einträge), sobald dein Gerät synchronisiert hat.</p>':''}<div class="divider"></div><button type="button" class="button secondary wide" data-action="account-logout">Abmelden</button><p class="small muted">Lokale Lernstände bleiben auf diesem Gerät erhalten.</p></section>`;}
