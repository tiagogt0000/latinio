...
function latinioApi(request) {
  ...
  try {
    ...
    if(['profiles','profileCreate','profileDelete','profileCollections','shareRevoke','shareList','shareCreate','shareChanges','shareSend','logout'].includes(request.action))return accountApi_(request,identity);
